 ...
 * if available, using the Servlet 3.0 API to dynamically load and install the
 * tunnel servlet.
 *
 * Note that because Guacamole depends on the Servlet 2.5 API, and 3.0 may not
 * be available or needed if WebSocket is not desired, the 3.0 API is detected
 * and invoked dynamically via reflection.
 *
 * @author Michael Jumper
 */
public class WebSocketSupportLoader implements ServletContextListener {

  private static final Logger logger = Logger.getLogger(WebSocketSupportLoader.class.getName());
  ...
  public void contextInitialized(ServletContextEvent sce) {
    ...
    try {
      ...
      try {
        ...
        logger.log(Level.INFO, "Servlet API 3.0 not found.", e);
      } catch (NoSuchMethodException e) {
        logger.log(Level.WARNING, "Servlet API 3.0 found, but incomplete.", e);
      } // Servlet API 3.0 found, but errors during use
      catch (IllegalAccessException e) {
        logger.log(Level.SEVERE, "Unable to load WebSocket tunnel servlet.", e);
      } catch (InvocationTargetException e) {
        logger.log(Level.SEVERE, "Internal error loading WebSocket tunnel servlet.", e);
      }
      ...
      logger.log(Level.SEVERE, "Unable to load/detect WebSocket support.", e);
    }
